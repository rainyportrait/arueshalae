...
export class Favorites {
	...
	private async showPostCountDiff() {
		const [totalFavorites, apiCount] = await Promise.all([this.getTotalFavorites(), getApiPostCount()])
	...
	private async getTotalFavorites(): Promise<number> {
		...
		const element = document.querySelector('a[href*="index.php?page=favorites&s=view&id="]')
	...
	private async getIdsFromPage(pid: number): Promise<number[]> {
		const dom = await fetchDocument(`/index.php?page=favorites&s=view&id=${this.getUserId()}&pid=${pid}`)

...
async function getUserID(): Promise<number> {
	...
	const document = await fetchDocument(`https://rule34.xxx/index.php?page=account&s=home`)
	const favoritesUrl = document.querySelector('a[href*="index.php?page=favorites"]')?.getAttribute("href")
	if (!favoritesUrl) {

...
CREATE TABLE posts (
 ...
 availability TEXT NOT NULL DEFAULT 'unknown' CHECK(availability IN ('unknown','available','deleted')),
 details_checked_at INTEGER,
 first_seen_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);
CREATE TABLE favorites (
 post_id INTEGER PRIMARY KEY NOT NULL REFERENCES posts(post_id),
 membership TEXT NOT NULL DEFAULT 'unknown' CHECK(membership IN ('unknown','favorited','unfavorited')),
...
INSERT INTO posts(post_id,first_seen_at) SELECT external_id,added_at FROM legacy_posts;
INSERT INTO favorites(post_id) SELECT external_id FROM legacy_posts;
INSERT INTO post_media(post_id,storage_name,extension,mime,original,downloaded_at)
 SELECT external_id,printf('%07d_%d.%s',id,external_id,extension),extension,mime,original,added_at FROM legacy_posts;
INSERT INTO post_tags SELECT p.external_id,t.tag_id FROM legacy_post_tags t JOIN legacy_posts p ON p.id=t.post_id;
DROP TABLE legacy_post_tags;
DROP TABLE legacy_posts;
CREATE INDEX IDX_post_tags_tag_id ON post_tags(tag_id);
CREATE VIEW tags_with_uses AS SELECT t.tag_id,t.name,t.kind,COUNT(*) uses FROM tags t JOIN post_tags pt USING(tag_id) GROUP BY t.tag_id;
...
CREATE TABLE sync_runs (
 ...
 status TEXT NOT NULL DEFAULT 'pending' CHECK(status IN ('pending','running','unresolved','cancelled','complete')),
 phase TEXT NOT NULL DEFAULT 'scan' CHECK(phase IN ('scan','verify','classify')),
...
CREATE TABLE sync_order_entries (
 ...
 post_id INTEGER NOT NULL REFERENCES posts(post_id),
 PRIMARY KEY(sync_run_id,position), UNIQUE(sync_run_id,post_id)
);
...
CREATE TABLE worker_leases (
 worker_kind TEXT PRIMARY KEY CHECK(worker_kind IN ('reconciliation','download')),

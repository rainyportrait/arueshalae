...
import type { AutocompleteSuggestion } from "../../userscript/api/autocomplete.ts"
import { resetDom } from "../dom.ts"
import { deferred } from "../helpers/deferred.ts"

const api = vi.hoisted(() => ({ fetchAutocomplete: vi.fn() }))
vi.mock("../../userscript/api/autocomplete.ts", () => ({
...
describe("AutocompleteInput", () => {
    ...
    async function makeInput(): Promise<HTMLInputElement> {
        const { AutocompleteInput } = await import("../../userscript/AutocompleteInput.ts")
